...
q13_rd_stock_crop(i2)..
  sum((ct,supreg(h2,i2)), v13_tau_core(h2,"crop")**(i13_tc_exponent(ct)/0.6)) =e= (1-0.2) * (vm_rd_stock_per_area(i2, "crop") * i13_tc_exponent(ct) / sum(ct, i13_tc_factor(ct)))**(1/0.6) +
                                                                                  + 0.2 * (pc13_rd_stock_per_area_global("crop") * i13_tc_exponent(ct) / sum(ct, i13_tc_factor(ct)))**(1/0.6) +;
