...
*' Risk aversity incentivizes a mix of crops and irrigation strategies.
  q30_prod_risk_aux(j2) ..
    v30_prod_risk_aux(j2) * sum((kcr,w), vm_area(j2,kcr,w) * vm_yld(j2,kcr,w) * fm_prices_initial(kcr))  =e= sum((kcr,w), vm_area(j2,kcr,w)**2 * vm_yld(j2,kcr,w)**2 * fm_prices_initial(kcr)**2 * p30_prod_stddev(j2,kcr,w)**2);

  q30_prod_risk(j2) ..
    v30_prod_risk(j2) =e= sum(cell(i2,j2), p30_risk_aversity(i2)) * v30_prod_risk_aux(j2);
